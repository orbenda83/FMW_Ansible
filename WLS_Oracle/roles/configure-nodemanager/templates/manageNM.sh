...
NodeManagerLockFile={{ wls_domain_home }}/nodemanager/nodemanager.process.lck
exec={{ wls_domain_home }}/bin/startNodeManager.sh
prog='nodemanager'
user={{ oracle_user }}
...
is_nodemgr_running() {
    local nodemgr_cnt=`ps -ef        | \
        grep 'java'                  | \
        grep 'weblogic.NodeManager'  | \
        grep '{{ wls_domain_home }}'  | \
        grep -v grep                  | \
        wc -l`
    echo $nodemgr_cnt
}
